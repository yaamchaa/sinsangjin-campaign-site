...
import { InsertNewsItem, InsertProposal, InsertUser, newsItems, proposals, users } from "../drizzle/schema";
...
// News Items (뉴스룸)
// =========================
export async function listNewsItems(limit = 100) {
  const db = await getDb();
  if (!db) return [];
...
export async function getNewsCount() {
  const db = await getDb();
  if (!db) return 0;
  const result = await db.select().from(newsItems);
